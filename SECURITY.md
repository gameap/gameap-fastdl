...
- Content is never executed on the server. HTML autoindex values are escaped; a restrictive content security policy is set. File response types are fixed rather than sniffed. Only approved, uncompressed files in `motd/` are served inline for browser display. MOTD pages remain sandboxed: styles and images from the same origin, inline styles and data images are allowed, while scripts, frames, forms and objects are blocked. Other assets and all `.bz2` downloads remain attachments.
...
Tests cover allowlist decisions, MOTD MIME types and response policies, traversal and encoded paths, Windows device names/streams, internal and external symlinks, directory replacement races, hardlinks, FIFOs, sockets, forbidden autoindex entries, `.cfg.bz2`, automatic compression updates, range/HEAD responses and configuration revocation. Windows-specific filesystem behavior requires running tests on Windows; cross-compilation alone does not verify kernel behavior. The CI matrix includes a native Windows job.
